...
class Generator(nn.Module):
    def __init__(self, noise_dim, condition_dim, output_channels=2, img_size=(128, 128)):
...
class Discriminator(nn.Module):
    def __init__(self, condition_dim, output_channels=2, img_size=(128, 128)):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(output_channels, img_size[0], kernel_size=4, stride=2, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(img_size[0], img_size[0]*2, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(img_size[0]*2),
            nn.LeakyReLU(0.2, inplace=True),
        )

        # Transformer layer
        self.transformer_layer = TransformerEncoderLayer(d_model=img_size[0]*2, nhead=4)
        self.transformer = TransformerEncoder(self.transformer_layer, num_layers=2)

        # Final layers
        self.fc = nn.Sequential(
            nn.Linear(img_size[0]*2 * img_size[1]//4 * img_size[1]//4 + condition_dim, 1),
            nn.Sigmoid()
        )
