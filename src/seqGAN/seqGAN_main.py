import argparse
import os
from seqGAN_utils import load_data, prepare_data, save_model, save_result, make_batch, acc_func, loss_func
from seqGAN import GAN_Model

import numpy as np
from sklearn.model_selection import train_test_split
import time
"""
Code first used from: https://github.com/mshadloo/Neural-Machine-Translation-with-Attention/blob/master/main.py
"""
#turned to a function for ease of viewing. Feel free to unpack
def make_parser():
...
def train_one_epoch(epoch, model,X_train, Y_train):
    ...
    for batch, inp, targ in dataset:
        ...
        acc += (acc_b * len(inp))
        if batch % 100 == 0:
            print('Epoch {} Batch {} Loss {:.4f} Accuracy {:.4f}'.format(epoch + 1,batch, loss / data_count, acc / data_count))
